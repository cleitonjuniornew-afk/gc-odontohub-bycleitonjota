...
export function SignUpForm() {
  ...
  async function onSubmit(data: SignUpInput) {
    if (!isSupabaseConfigured) {
      toast.info("Ambiente de demonstração: conecte o Supabase para ativar o cadastro real.");
    ...
    if (error) {
      toast.error("Não foi possível concluir o cadastro. Vamos tentar novamente.");
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-text-secondary">Comece a organizar sua vida acadêmica.</p>
      </div>
      ...
      <div>
        ...
        <Input id="nome" icon={<User className="h-4 w-4" />} placeholder="Seu nome" className="mt-1.5" {...register("nome")} />
      ...
      <div>
        ...
        <Input id="confirmPassword" type="password" icon={<Lock className="h-4 w-4" />} placeholder="••••••••" className="mt-1.5" {...register("confirmPassword")} />
      ...
      <p className="text-center text-sm text-text-secondary">
        Já tem conta? <Link href="/login" className="font-medium text-secondary hover:underline">Entrar</Link>
      </p>
    </form>
  );
}
