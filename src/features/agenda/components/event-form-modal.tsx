...
export function EventFormModal({ open, onOpenChange, onSubmit, submitting, initialData, defaultDate }: Props) {
  const { register, control, handleSubmit, reset, formState: { errors } } = useForm<EventFormInput>({
  ...
  useEffect(() => {
    if (open) {
      if (initialData) {
        ...
        reset({ title: initialData.title, type: initialData.type, date, time, disciplineId: initialData.disciplineId ?? "" });
      } else {
        reset({ title: "", type: "evento", date: defaultDate ?? "", time: "09:00", disciplineId: "" });
      }
    }
  }, [open, initialData, defaultDate, reset]);

  async function submit(data: EventFormInput) {
    const start = new Date(`${data.date}T${data.time || "09:00"}:00`).toISOString();
    await onSubmit({
      title: data.title,
      type: data.type,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Provas, clínicas e aulas — tudo em um só calendário.</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit(submit)} className="space-y-4">
          <div>
            <Label htmlFor="title">Título</Label>
            <Input id="title" placeholder="Ex: Prova de Semiologia" className="mt-1.5" {...register("title")} />
          ...
          <div className="mt-2 flex justify-end gap-3">
            ...
            <Button type="submit" loading={submitting}>{initialData ? "Salvar alterações" : "Criar evento"}</Button>
          </div>
        </form>
      </DialogContent>
    </Dialog>
